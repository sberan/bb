...
// `acpLaunchSpec` site (thread.start, the resume contexts, thread.goal.clear,
// provider.list_models). The command schemas are strict, so an old daemon
// cannot parse a payload carrying the new field; the version mismatch is what
// triggers the enrolled daemon's automatic update instead of an
// `invalid-message` reconnect loop. (123 was taken on main by the
// engine-range enforcement on cached-artifact registrations.)
